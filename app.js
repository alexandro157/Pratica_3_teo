...
app.post('/save', upload.single('imagen'), async (req, res) => {
  ...
  try {
    ...
    await connection.promise().query(
      ...
      { nombre, apellidos, seccion, grado, tutor, imagen: s3Key }
    );
    res.redirect('/');
  } catch (error) {
...
app.post('/update', upload.single('imagen'), async (req, res) => {
  ...
  try {
    await connection.promise().query(
      'UPDATE estudiantes SET nombre = ?, apellidos = ?, seccion = ?, grado = ?, tutor = ?, imagen = ? WHERE id = ?',
      [nombre, apellidos, seccion, grado, tutor, imagen, id]
    );
    res.redirect('/');
  } catch (error) {
